 ...
 * @since      1.0
 */
class ThemeHandler extends AbstractHandler implements HandlerInterface
{
    /**
    ...
    {
        $theme = substr($resource_namespace, strrpos($resource_namespace, '/') + 1, 9999);

        $test = $theme;
        if (is_numeric($test) && (int)$test == $theme) {
        } else {
            $theme = $this->extensions->extensions[$this->catalog_type_id]->names[$test];
        }

        $temp = substr($resource_namespace, 0, strrpos($resource_namespace, '/') - 1);

        if (isset($this->extensions->extensions[$this->catalog_type_id]->ids[$theme])) {
            $this->extension = $this->extensions->extensions[$this->catalog_type_id]->extensions[$theme];
        }
    ...
    protected function searchResourceMap($resource_namespace, $multiple = false)
    {
        if (isset($this->resource_map[strtolower($resource_namespace)])) {
            ...
            /** Default location */
            $path = $this->base_path . '/Source/Themes/'
