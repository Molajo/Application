 ...
 * @since      1.0
 */
class Page extends AbstractAdapter implements AdapterInterface
{
    /**
    ...
    {
        $pageview = substr($resource_namespace, strrpos($resource_namespace, '/') + 1, 9999);

        $test = $pageview;
        if (is_numeric($test) && (int)$test == $pageview) {
        } else {
            $test     = strtolower($test);
            $pageview = $this->extensions->extensions[$this->catalog_type_id]->names[$test];
        }

        $temp = substr($resource_namespace, 0, strrpos($resource_namespace, '/') - 1);

        if (isset($this->extensions->extensions[$this->catalog_type_id]->ids[$pageview])) {
            $this->extension = $this->extensions->extensions[$this->catalog_type_id]->extensions[$pageview];
        }
    ...
    protected function searchResourceMap($resource_namespace, $multiple = false)
    {
        if (isset($this->resource_map[strtolower($resource_namespace)])) {
            ...
            /** Default location */
            $path = $this->base_folder . '/Source/Views/Pages/'
