 ...
 * @since      1.0
 */
class Wrap extends AbstractAdapter implements AdapterInterface
{
    /**
    ...
    {
        $wrapview = substr($resource_namespace, strrpos($resource_namespace, '/') + 1, 9999);

        $test = $wrapview;
        if (is_numeric($test) && (int)$test == $wrapview) {
        } else {
            $test     = strtolower($test);
            $wrapview = $this->extensions->extensions[$this->catalog_type_id]->names[$test];
        }

        $temp = substr($resource_namespace, 0, strrpos($resource_namespace, '/') - 1);

        if (isset($this->extensions->extensions[$this->catalog_type_id]->ids[$wrapview])) {
            $this->extension = $this->extensions->extensions[$this->catalog_type_id]->extensions[$wrapview];
        }
