...
class ExtensionMap implements ExtensionsInterface
{
    ...
    {
        $catalog_type_controller = $this->resource->get(
            'query:///Molajo//Model//Datasource//CatalogTypes.xml',
            array('Parameters' => $this->runtime_data)
        );

        $catalog_type_controller->setModelRegistry('check_view_level_access', 0);
        $catalog_type_controller->setModelRegistry('process_events', 0);
        $catalog_type_controller->setModelRegistry('query_object', 'list');
        $catalog_type_controller->setModelRegistry('use_pagination', 0);

        $catalog_type_controller->model->query->where(
            $catalog_type_controller->model->database->qn($catalog_type_controller->getModelRegistry('prefix', 'a'))
            . ' . '
            . $catalog_type_controller->model->database->qn('id')
            . ' IN ('
            ...
            . $catalog_type_controller->model->database->qn($catalog_type_controller->getModelRegistry('prefix', 'a'))
            . ' . '
            . $catalog_type_controller->model->database->qn('model_type')
            . ' = '
            . $catalog_type_controller->model->database->q('Resource')
            . ')'
        );
...
//        }
        ...
        foreach ($items as $item) {
            ...
            if ($catalog_type_id == (int)$this->runtime_data->reference_data->catalog_type_menuitem_id) {
        ...
        if ($catalog_type_model_name == 'View//page') {
            $catalog_type_model_name = 'View//Page';
        } elseif ($catalog_type_model_name == 'View//template') {
            $catalog_type_model_name = 'View//Template';
        } elseif ($catalog_type_model_name == 'View//wrap') {
            $catalog_type_model_name = 'View//Wrap';
        }
    ...
    protected function getCatalogResourceExtensions($catalog_type_id)
    {
        ...
        $extensions_controller->model->query->select(
            $extensions_controller->model->database->qn($extensions_controller->getModelRegistry('primary_prefix', 'a'))
            . ' . '
            . $extensions_controller->model->database->qn('id')
        );

        $extensions_controller->model->query->select(
            $extensions_controller->model->database->qn($extensions_controller->getModelRegistry('primary_prefix', 'a'))
            . ' . '
            . $extensions_controller->model->database->qn('alias')
        );

        $extensions_controller->model->query->where(
            $extensions_controller->model->database->qn($extensions_controller->getModelRegistry('primary_prefix', 'a'))
            . ' . '
            . $extensions_controller->model->database->qn('id')
            . ' = '
        ...
        $extensions_controller->model->query->where(
            $extensions_controller->model->database->qn($extensions_controller->getModelRegistry('primary_prefix', 'a'))
            . ' . '
            . $extensions_controller->model->database->qn('status')
            . ' > '
            . ' 0 '
        );

        $extensions_controller->model->query->order(
            $extensions_controller->model->database->qn($extensions_controller->getModelRegistry('primary_prefix', 'a'))
            . ' . '
            . $extensions_controller->model->database->qn('alias')
        );
    ...
    protected function getCatalogSystemExtensions($catalog_type_id)
    {
        ...
        $extensions_controller->model->query->select(
            $extensions_controller->model->database->qn($extensions_controller->getModelRegistry('primary_prefix', 'a'))
            . ' . '
            . $extensions_controller->model->database->qn('menu')
        );

        $extensions_controller->model->query->select(
            $extensions_controller->model->database->qn($extensions_controller->getModelRegistry('primary_prefix', 'a'))
            . ' . '
            . $extensions_controller->model->database->qn('path')
        );

        $extensions_controller->model->query->select(
            $extensions_controller->model->database->qn($extensions_controller->getModelRegistry('primary_prefix', 'a'))
            . ' . '
            . $extensions_controller->model->database->qn('page_type')
        );

        $extensions_controller->model->query->where(
            $extensions_controller->model->database->qn($extensions_controller->getModelRegistry('primary_prefix', 'a'))
            . ' . '
            . $extensions_controller->model->database->qn('catalog_type_id')
            . ' = '
            . (int)$catalog_type_id
        );
        ...
        $extensions_controller->model->query->where(
            ...
            . $extensions_controller->model->database->qn('id')
            . ' <> '
            . $extensions_controller->model->database->qn(
    ...
    protected function getExtension($id, $model_name, $resource_indicator = false)
    {
        ...
        $item_resource->model->query->where(
            $item_resource->model->database->qn($item_resource->getModelRegistry('primary_prefix', 'a'))
            . ' . '
            . $item_resource->model->database->qn('id')
            . ' = '
            . (int)$id
        );
        ...
        try {
            ...
            echo 'RenderingExtensionsServiceProvider: Extension not found: ' . $model_name;
            throw new RuntimeException ($e->getMessage());
        }
