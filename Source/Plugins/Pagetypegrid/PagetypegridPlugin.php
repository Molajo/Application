...
class PagetypegridPlugin extends DisplayEventPlugin implements DisplayInterface
{
    ...
    protected function getGridData()
    {
        ...
        $grid->where(
            'column',
            $grid->getModelRegistry('primary_prefix') . '.' . 'catalog_type_id',
            '=',
            'integer',
            (int)$catalog_type_id
        );
        ...
        if ($list == '' || trim($list) == '' || $list === null) {
            ...
            $grid->where(
                'column',
                $grid->getModelRegistry('primary_prefix') . '.' . 'status',
                'IN',
                'integer',
                $list
            );
        ...
        if ((int)$parameters->grid_pagination_use == 1) {
            ...
            $grid->setModelRegistry('model_count', 20);
        ...
        $this->plugin_data->grid_model_registry = $grid->getModelRegistry('*');
