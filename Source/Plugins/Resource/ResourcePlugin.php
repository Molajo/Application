 ...
 */
namespace Molajo\Plugins\Resource;
...
class ResourcePlugin extends SystemEventPlugin implements SystemInterface
{
    ...
    {
        $model_type  = $this->runtime_data->route->b_model_type;
        $model_name  = $this->runtime_data->route->b_model_name;
        $sef_request = $this->runtime_data->route->sef_request;

        if ($model_type == 'Resource') {
            $model = 'Molajo//' . $model_name . '//Configuration.xml';
        } else {
            $model = 'Molajo//' . $sef_request . '//Configuration.xml';
        }
...
//        die;
    ...
    protected function getResourceList()
    {
        $this->resource->where(
            'column',
            $this->resource->getModelRegistry('primary_prefix', 'a')
            . '.'
            . $this->resource->getModelRegistry('primary_key', 'id'),
            '=',
            'integer',
            (int)$this->runtime_data->route->source_id
        );
    ...
    protected function getResourceMenuItem()
    {
        ...
        $this->plugin_data->resource->menuitem->model_registry = $controller->getModelRegistry('*');
