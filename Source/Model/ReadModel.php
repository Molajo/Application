...
use CommonApi\Model\ReadInterface;
 ...
 * @since      1.0
 */
class ReadModel extends Model implements ReadInterface
{
    /**
...
//            } else {
        ...
        if ($cached_output === false) {
            ...
            if ($count < count($query_results)) {
                ...
                $this->model_registry['total_items'] = $this->database->loadResult();
                $this->query                         = $hold;
            } else {
            ...
            if (is_array($criteria) && count($criteria) > 0) {
                ...
                foreach ($criteria as $where) {
                    ...
                    if (isset($where['name'])
                        ...
                        $this->query->where(
                            $this->database->qn($where['name'])
                            . $where['connector']
                            . $this->database->qn($where['name2'])
                        );
                    }
                }
            }
        ...
        if (isset($this->model_registry['criteria_status'])) {
            if ((int)$this->model_registry['criteria_status'] === null) {
                ...
                $this->query->where(
                    ...
                    . ' IN (' . (string)$this->model_registry['criteria_status'] . ')'
                );
            }
        }
