...
abstract class Set extends Base
{
    ...
    /**
     * Get matching application array entry or get default
     *
    ...
    {
        if ($app->base_path === '') {
            $this->application_base_path = '';
            $this->path                  = $this->request_path;
        } else {
            $this->application_base_path = $app->base_path;
            $this->path                  = substr($this->request_path, strlen(trim($this->application_base_path)) + 1,
                999);
        }
