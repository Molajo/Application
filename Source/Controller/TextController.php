...
class TextController implements TextInterface
{
    ...
    public function getDatalist(
        ...
        if (count($fields) < 2) {
            ...
            $query->model->query->select(
                'DISTINCT '
                . $query->model->database->qn($primary_prefix . '.' . $primary_key) . ' as id'
            );

            $query->model->query->select(
                $query->model->database->qn(
                    $primary_prefix
                    . '.' . $name_key
                ) . ' as value'
            );
            ...
            foreach ($fields as $field) {
                ...
                $query->model->query->select(
                    $distinct . ' '
                    . $query->model->database->qn($alias . '.' . $name) . ' as ' . $as
                );
            }

            $query->model->query->order($query->model->database->qn($ordering) . ' ASC');
        }
     ...
     * @since   1.0
     */
    protected function setWhereCriteria($field, $value, $alias, $query)
    {
        if (strrpos($value, ',') > 0) {
            $query->model->query->where(
                $query->model->database->qn($alias . '.' . $field)
                . ' IN (' . $value . ')'
            );
        } elseif ((int)$value == 0) {
        } else {
            $query->model->query->where(
                $query->model->database->qn($alias . '.' . $field) . ' = ' . (int)$value
            );
        }
    ...
    protected function publishedStatus(array $query = array(), $primary_prefix = 'a')
    {
        ...
        $query->model->query->where(
            '(' . $query->model->database->qn($primary_prefix)
            . '.' . $query->model->database->qn('start_publishing_datetime')
            . ' = ' . $query->model->database->q($query->model->null_date)
            . ' OR ' . $query->model->database->qn($primary_prefix)
            . '.' . $query->model->database->qn('start_publishing_datetime')
            . ' <= ' . $query->model->database->q($query->model->now) . ')'
        );

        $query->model->query->where(
            '(' . $query->model->database->qn($primary_prefix)
            . '.' . $query->model->database->qn('stop_publishing_datetime')
            . ' = ' . $query->model->database->q($query->model->null_date)
            . ' OR ' . $query->model->database->qn($primary_prefix)
            . '.' . $query->model->database->qn('stop_publishing_datetime')
            . ' >= ' . $query->model->database->q($query->model->now) . ')'
