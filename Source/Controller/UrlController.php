...
class UrlController implements UrlInterface
{
    ...
    public function get($request_type = 1, $catalog_type_id = null, $source_id = null, $url_sef_request = null)
    {
        ...
        if ($request_type == 1) {
            ...
            $query->model->query->select(
                $query->model->database->qn($prefix)
                . '.'
                . $query->model->database->qn($key)
            );
            /** Get Redirect ID */
            ...
            $query->model->query->select(
                $query->model->database->qn($prefix)
                . '.'
                . $query->model->database->qn('redirect_id')
            );
            /** Get SEF Request */
            ...
            $query->model->query->select(
                $query->model->database->qn($prefix)
                . '.'
                . $query->model->database->qn('sef_request')
            );
        ...
        if ($url_sef_request === null) {
            ...
            $query->model->query->where(
                $query->model->database->qn($prefix)
                . '.'
                . $query->model->database->qn('source_id')
                . ' = '
                . (int)$source_id
            );
        } else {
            ...
            $query->model->query->where(
                ...
                . $query->model->database->qn('sef_request')
                . ' = '
                . $query->model->database->q($url_sef_request)
            );
        }
        ...
        $query->model->query->where(
            $query->model->database->qn($prefix)
            . '.'
            . $query->model->database->qn('application_id')
            . ' = '
            . $query->model->database->q($this->application_id)
        );
    ...
    {
        if (substr($url, 0, strlen(BASE_FOLDER)) == BASE_FOLDER) {
            return false;
        } elseif ((strtolower(substr($url, 0, 3)) == 'www')
            && (substr($url, 3, strlen(BASE_FOLDER)) == BASE_FOLDER)
        ) {
            return false;
        } else {
            return true;
