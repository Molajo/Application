...
use CommonApi\Application\FrontControllerInterface;
 ...
 * @since      1.0.0
 */
final class FrontController implements FrontControllerInterface, ErrorHandlingInterface
{
    /**
     * First Step
     *
    ...
    protected function runStep($factory_method)
    {
        ...
        if (isset($results->error_code) && (int)$results->error_code > 0) {
            trigger_error('Frontcontroller::runStep Error for: ' . $factory_method, E_USER_WARNING);
        }
    ...
    protected function start()
    {
        ...
        if (count($this->requests) > 0) {
            foreach ($this->requests as $request) {
                $this->scheduleFactoryMethod($request, 'Start');
            }
        }

        $this->scheduleEvent($event_name = 'onAfterStart');
